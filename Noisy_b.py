...
for M in range(2, N):
    ...
    for num_s in range(1, M):
        ...
        for i in range(800):
            ...
            x = sparse_x(N, num_s)
            noise = np.random.normal(0, 0.01, size=(M, 1))
            Noise.append(noise)
            X.append(x)

            y = np.dot(A_norm, X[0]) + Noise[0]

            x_est = OMP(A_norm, y, num_s, Noise[0])
...
ax1.set_title("Noisy_norm_n_N100")
