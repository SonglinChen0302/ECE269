...
for M in range(2, N):
    ...
    # success_vector = [0] * (48 - len(success_vector)) + success_vector
    # Pro.append(success_vector[::-1])

    avg_err_vector = avg_err_vector[::-1]
    avg_err_vector = [0] * (48 - len(avg_err_vector)) + avg_err_vector
...
# M = np.linspace(1, 48, 48, dtype=int)
# S = np.linspace(1, 48, 48, dtype=int)
# fig1, ax1 = plt.subplots(1)
# im1 = ax1.imshow(Probability)
#
# ax1.set_xticks(np.arange(48), minor=False)
# ax1.set_yticks(np.arange(48), minor=False)
...
ax2.set_xticks(np.arange(48), minor=False)
ax2.set_yticks(np.arange(48), minor=False)
...
ax1.set_title("noiseless phase transition N=50")
# ax2.set_title("Normalized Error transition plot N=50")
